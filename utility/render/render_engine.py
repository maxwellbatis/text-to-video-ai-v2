...
def get_output_media_vsl(audio_file_path, script, background_video_data, video_server, template_config=None):
    ...
    if template_config:
        ...
        for overlay in text_overlays:
            # Criar TextClip com estilo VSL
            style = overlay['style']
            text_clip = (TextClip(txt=overlay['text'],
                                  fontsize=int(style['size'].split('-')[0]) if '-' in style['size'] else int(style['size']),
                                  font=style['font'],
                                  color=style['color'],
                                  stroke_color=style['stroke'],
                                  stroke_width=style['stroke_width'],
                                  method="label",
                                  bg_color="rgba(0,0,0,0.8)",  # Fundo mais escuro para VSL
                                  size=(1920, None))
                      .set_start(overlay['start_time'])
                      .set_end(overlay['end_time'])
                      .set_position(("center", overlay['position']))
                      .margin(bottom=200 if overlay['position'] == 'bottom' else 50)
                      .crossfadein(0.5)
                      .crossfadeout(0.5))
            visual_clips.append(text_clip)
    else:
